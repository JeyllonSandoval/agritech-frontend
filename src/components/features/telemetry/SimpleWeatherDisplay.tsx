// ============================================================================
// WEATHER DATA DISPLAY FOR GROUPS
// Component for displaying weather data and forecasts for groups (exact copy of WeatherDataDisplay)
// ============================================================================

import React from 'react';
import { WeatherData } from '../../../types/telemetry';
import { 
  WiDaySunny,
  WiDayCloudy,
  WiDayRain,
...
interface SimpleWeatherDisplayProps {
  ...
  variant?: 'full' | 'compact' | 'inline' | 'realtime';
  showHeader?: boolean;
...
const SimpleWeatherDisplay: React.FC<SimpleWeatherDisplayProps> = ({
  ...
  const getWeatherIcon = (weatherId: number, size: 'sm' | 'md' | 'lg' = 'md') => {
    ...
    if (weatherId >= 200 && weatherId < 300) {
      return <WiDayThunderstorm className={`${sizeClasses[size]} text-amber-500`} />;
    }
    if (weatherId >= 300 && weatherId < 400) {
      return <WiDaySprinkle className={`${sizeClasses[size]} text-sky-400`} />;
    }
    if (weatherId >= 500 && weatherId < 600) {
      return <WiDayRain className={`${sizeClasses[size]} text-blue-500`} />;
    }
    if (weatherId >= 600 && weatherId < 700) {
      return <WiDaySnow className={`${sizeClasses[size]} text-slate-300`} />;
    }
    if (weatherId >= 700 && weatherId < 800) {
      return <WiFog className={`${sizeClasses[size]} text-gray-500`} />;
    }
    if (weatherId === 800) {
      return <WiDaySunny className={`${sizeClasses[size]} text-yellow-400`} />;
    }
    if (weatherId >= 801 && weatherId < 900) {
      return <WiDayCloudy className={`${sizeClasses[size]} text-gray-400`} />;
    }
    return <WiDaySunny className={`${sizeClasses[size]} text-gray-400`} />;
  };
  ...
  if (loading) {
    return (
      <div className={`bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 shadow-lg ${className}`}>
  ...
  // Renderizado para tiempo real (usado en RealtimeDataDisplay)
  if (variant === 'realtime') {
    return (
      <div className={`bg-gradient-to-br from-blue-900/40 to-cyan-800/30 rounded-2xl p-6 mt-4 border border-cyan-400/20 shadow-xl mb-8 flex flex-col md:flex-row items-center md:items-stretch gap-6 ${className}`}>
        {/* Icono y temperatura */}
        <div className="flex flex-col items-center justify-center md:justify-between md:items-center min-w-[180px]">
          <div>
            {getWeatherIcon(weather.id, 'lg')}
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg mx-auto">
          <div className="rounded-lg px-4 py-2 flex flex-col items-center">
            ...
            <span className="text-white font-semibold text-base md:text-lg">{Math.round(current.feels_like)}°C</span>
          </div>
          <div className="rounded-lg px-4 py-2 flex flex-col items-center">
            <WiHumidity className="w-8 h-8 text-white/60 mb-1" />
            <span className="text-xs text-white/60">Humedad</span>
            <span className="text-white font-semibold text-base md:text-lg">{current.humidity}%</span>
          </div>
          <div className="rounded-lg px-4 py-2 flex flex-col items-center">
            <WiBarometer className="w-8 h-8 text-white/60 mb-1" />
            <span className="text-xs text-white/60">Presión</span>
            <span className="text-white font-semibold text-base md:text-lg">{current.pressure} hPa</span>
          </div>
          <div className="rounded-lg px-4 py-2 flex flex-col items-center">
            <MdOutlineVisibility className="w-8 h-8 text-white/60 mb-1" />
            <span className="text-xs text-white/60">Visibilidad</span>
            <span className="text-white font-semibold text-base md:text-lg">{(current.visibility / 1000).toFixed(1)} km</span>
          </div>
        </div>
      </div>
    );
  ...
  // Renderizado compacto para dispositivos individuales
  if (variant === 'compact') {
    return (
      <div className={`mt-4 bg-gradient-to-br from-blue-900/40 to-cyan-800/30 rounded-2xl p-6 border border-cyan-400/20 shadow-xl flex flex-col md:flex-row items-center md:items-stretch gap-6 ${className}`}>
        {/* Icono y temperatura */}
        <div className="flex flex-col items-center justify-center md:justify-between md:items-center min-w-[180px]">
          <div>
            {getWeatherIcon(weather.id, 'lg')}
  ...
  // Renderizado inline para grupos
  if (variant === 'inline') {
    return (
      <div className={`bg-gradient-to-br from-blue-900/40 to-cyan-800/30 rounded-2xl p-6 border border-cyan-400/20 shadow-xl flex flex-col md:flex-row items-center md:items-stretch gap-6 ${className}`}>
        {/* Icono y temperatura */}
        <div className="flex flex-col items-center justify-center md:justify-between md:items-center min-w-[180px]">
          <div>
            {getWeatherIcon(weather.id, 'lg')}
  ...
  return (
    <div className={`bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 shadow-lg ${className}`}>
      ...
      <div className="space-y-6">
        ...
        {/* Hourly Forecast */}
        {showForecasts && weatherData.hourly && weatherData.hourly.length > 0 && (
        ...
        {/* Daily Forecast */}
        {showForecasts && weatherData.daily && weatherData.daily.length > 0 && (
          <div className="bg-white/10 border border-white/20 rounded-xl p-6 backdrop-blur-sm">
